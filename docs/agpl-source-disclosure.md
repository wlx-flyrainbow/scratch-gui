# AGPL 源码公开文案（已按当前项目填写）

本项目当前信息：

- 项目名：`scratch-gui`
- 许可证：`AGPL-3.0-only`
- 仓库地址：`https://github.com/wlx-flyrainbow/scratch-gui`
- 问题反馈：`https://github.com/wlx-flyrainbow/scratch-gui/issues`

---

## 1) 网站页脚文案（短）

```text
本服务基于 AGPLv3 许可的软件提供。该服务当前运行版本的完整对应源代码（含修改）可在此获取：
https://github.com/wlx-flyrainbow/scratch-gui
```

---

## 2) 关于页 / 法律声明文案（长）

```text
开源许可声明（AGPLv3）

本在线服务包含并基于 GNU Affero General Public License v3.0（AGPLv3）许可的软件进行开发与运行。
根据 AGPLv3 的要求，我们向所有通过网络与本服务交互的用户提供本服务当前运行版本的完整对应源代码（包括我们所作修改）。

源代码获取地址：
https://github.com/wlx-flyrainbow/scratch-gui

如你在获取或构建源代码时遇到问题，可通过以下方式联系我们：
https://github.com/wlx-flyrainbow/scratch-gui/issues
```

---

## 3) 仓库 `NOTICE` 模板（英文）

```text
NOTICE

This service includes software licensed under the GNU Affero General Public License v3.0 (AGPLv3).

Corresponding Source for the running service (including local modifications) is available at:
https://github.com/wlx-flyrainbow/scratch-gui

Contact / issue tracker:
https://github.com/wlx-flyrainbow/scratch-gui/issues
```

---

## 4) 可直接粘贴的 HTML 片段（可选）

```html
<footer>
  <p>
    本服务基于 AGPLv3 许可的软件提供。完整对应源代码（含修改）：
    <a href="https://github.com/wlx-flyrainbow/scratch-gui" target="_blank" rel="noopener noreferrer">
      https://github.com/wlx-flyrainbow/scratch-gui
    </a>
  </p>
</footer>
```

---

## 5) 上线前核对清单

- 链接指向当前线上运行版本对应的代码，而非过期代码。
- 保留上游版权声明与 `LICENSE` 文件。
- 你自己的修改也应包含在公开代码中。
- 提供基本构建/运行说明，便于用户获取“对应源码”。
