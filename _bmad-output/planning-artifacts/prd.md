...
知萌是一款面向 6-10 岁儿童与家庭用户的少儿创意编程启蒙桌面应用，首发优先服务 6-8 岁少儿编程启蒙家庭。产品基于 Scratch GUI 分叉进行品牌化与商业化改造，核心价值是让孩子用积木编程完成故事、动画和小游戏，同时让家长可以通过账号订阅获得可控、可交付的学习工具。

本阶段不追求完整社区生态，而是优先完成一个能真实售卖、能开通授权、能交付安装包、能被运营确认订单的 MVP 闭环。作为售卖型产品，未登录或未付费用户不应默认进入完整编辑器；客户端应先进入知萌锁定体验，只开放登录、注册、购买、授权刷新和必要的产品说明。
...
1. 用户打开官网下载页，下载知萌 Windows 客户端。
2. 用户安装并启动客户端。
3. 客户端显示知萌锁定体验，不展示完整编辑器操作区。
4. 用户进入知萌账号登录/注册。
...
- 将上游 Scratch GUI 包装为知萌桌面应用。
- 完成 AGPL 合规披露与源码链接。
- 建立知萌商标与品牌资产的基础防守，避免真实售卖后出现抢注、近似冲突或被迫改名。

商标与品牌管理要求：

- 首批真实售卖前完成“知萌”文字商标在第 9 类、第 41 类、第 42 类的近似检索与申请决策。
...
- 品牌使用统一为“知萌”，不得把 Scratch 作为产品名或售卖品牌；Scratch 只出现在 AGPL 合规、源码披露和必要技术说明中。

验收口径：

- `package.json` 中 appId、productName 指向知萌。
- Electron 窗口标题显示“知萌”。
- 官网下载页展示知萌品牌。
- 下载页和客户端帮助菜单都能打开当前版本对应源码链接。
- `docs/agpl-source-disclosure.md` 和 `docs/zhimeng-agpl-compliance.md` 可作为上线合规材料。
- 发布前已有商标近似检索记录、申请类别决策和品牌资产清单。

### Milestone 1: 账号授权

目标：

- 完成知萌账号登录、刷新、登出、授权拉取、离线租约和能力门禁。
...
- 未登录、未订阅、待确认、已过期、租约过期用户不能进入完整编辑器，只能进入锁定体验和购买流程。
...
- 真库验收脚本验证真实知萌页面内容，不只检查 HTTP 200。
...
- 密码输入框。
- 登录按钮。
- 客户端内置注册：用户可直接创建知萌账号，注册成功后进入已登录但未订阅状态。
- 错误提示。
- 关闭按钮。
